...
1. **Schema Registry（Schema 注册表）** —— 注册会话级别的 schema
2. **Schema Discovery（Schema 发现）** —— 查询当前会话中可用的 schema
...
### 3. Session 级 Schema 作用域

- Schema ID 仅在**会话维度唯一**
- Schema 的注册、发现、生命周期都限定在 session 范围内
- 不支持跨 session 的 schema 共享
...
Echo MCP 暴露 4 个 MCP 工具：

### `register_schema`

在当前会话中注册一个 JSON Schema，供后续校验使用。
...
列出当前会话中所有可用的 schema（包括内置的 `__schemaless__`）。
...
**输入：** `{"schema_id": "report_outline_v1"}`
