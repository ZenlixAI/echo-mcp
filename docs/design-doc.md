...
1. **Built-in Schema Config**：通过配置文件维护内置 schema
2. **Schema Discovery**：查询当前可用的内置 schema
...
### 目标 3：支持内置 schema 管理
schema 通过服务端配置文件维护，并支持 discovery 与校验。
...
### A. Built-in Schema Config
从配置文件加载内置 schema 并构建只读 registry。

### B. Validation Engine
将内置 schema 转换为可执行校验器，对 echo 请求进行验证。
...
## 7.1 Schema
用于约束 payload 结构的定义。  
在 Echo MCP 中，schema 为服务端配置维护的内置对象。
...
Echo MCP 的 schema 来源于服务端配置文件。  
不同 session 共享同一套内置 schema 集合。

## 8.2 持久化边界

配置文件中的 schema 在服务生命周期内保持可查询。  
若配置变更，需重启服务或触发显式 reload（实现可选）。
...
仅用于服务端加载配置时检测到重复 schema id 的实现内部错误（客户端通常不可见）。

## 11.3 `SCHEMA_NOT_FOUND`
请求使用了不存在的 schema id。

## 11.4 `INVALID_SCHEMA_DEFINITION`
服务端内置 schema 定义非法，无法接受或无法编译。
...
推荐内置 schema 配置使用 JSON Schema，原因：
...
推荐以 JSON Schema 作为“内置 schema 配置的官方格式”。  
内部实现可以：
...
## 24.4 discovery 必须可用
既然 schema discovery 是需求的一部分，就不应省略 `list_schemas` / `get_schema`。
...
1. 启动时加载合法内置 schema 成功
2. 重复内置 schema id 导致加载失败
3. 查询已有 schema 成功
4. 查询不存在 schema 失败
5. 按 schema echo 成功
